    ...
    **/
    function request($method, $url, $vars = array()) {
        ...
        if ($response) {
            ...
            $response->statusCode = curl_getinfo($this->request, CURLINFO_HTTP_CODE);
            $header_size = curl_getinfo($this->request, CURLINFO_HEADER_SIZE);
//            $response->headers = substr($response, 0, $header_size);
//            $response->body = substr($response, $header_size);
        } else {
    ...
    **/
     ...
     */
    function download($url, $folderSave, $fileName, $vars = array()){
        ...
        curl_setopt($this->request,CURLOPT_URL,$url);
        curl_setopt($this->request, CURLOPT_TIMEOUT, 3600);
        # set return transfer to false
        curl_setopt( $this->request, CURLOPT_RETURNTRANSFER, true );
        curl_setopt( $this->request, CURLOPT_BINARYTRANSFER, true );
        curl_setopt( $this->request, CURLOPT_SSL_VERIFYPEER, false );
        curl_setopt($this->request, CURLOPT_FILE, $fp);
        curl_setopt($this->request,CURLOPT_USERAGENT,'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.8.1.13) Gecko/20080311 Firefox/2.0.0.13');

        $response = curl_exec($this->request);
        if ($response) {
//            file_put_contents($file, $response);
            $response = new MyCurlFile($fileName, $file);
        } else {
            $this->error = curl_errno($this->request).' - '.curl_error($this->request);
            curl_close($this->request);
            fclose( $fp );
